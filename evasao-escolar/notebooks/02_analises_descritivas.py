...
Saídas:
  ...
  - Figuras salvas em: reports/figuras/  (prefixo A*)
"""
...
def carregar_painel() -> pd.DataFrame:
    return carregar_parquet(PARQUET_PATH, "python notebooks/01_exploracao_inicial.py")


# =============================================================================
# ENRIQUECIMENTO DE FEATURES DERIVADAS
# =============================================================================

def adicionar_razoes_operacionais(df: pd.DataFrame) -> None:
...
def adicionar_indice_infraestrutura(df: pd.DataFrame) -> None:
    cols_presentes = [c for c in COLUNAS_INFRA if c in df.columns]
    df["indice_infra"] = df[cols_presentes].mean(axis=1)


def adicionar_porte_escola(df: pd.DataFrame) -> None:
...
def adicionar_rotulos_geograficos(df: pd.DataFrame) -> None:
    df["localizacao_txt"] = df["TP_LOCALIZACAO"].map(LABELS_LOCALIZACAO)
    df["mesorregiao_txt"] = df["CO_MESORREGIAO"].map(NOMES_MESORREGIOES).fillna(
...
def enriquecer(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    adicionar_razoes_operacionais(df)
    adicionar_indice_infraestrutura(df)
    adicionar_porte_escola(df)
    adicionar_rotulos_geograficos(df)
    return df
...
def relatorio_dimensoes_painel(df: pd.DataFrame) -> None:
    ...
    for ano, n in df.groupby("NU_ANO_CENSO").size().items():
        print(f"  {ano}: {n:,} escolas")


def relatorio_estabilidade_longitudinal(df: pd.DataFrame) -> None:
    print("\nEstabilidade longitudinal (quantas escolas em N anos):")
    estabilidade = df.groupby("CO_ENTIDADE")["NU_ANO_CENSO"].nunique().value_counts().sort_index()
    for n_anos, n_escolas in estabilidade.items():
    ...
    print(f"\n  Escolas que sairam do painel (2022->2024): {len(escolas_2022 - escolas_2024):,}")
    print(f"  Escolas que entraram no painel (2022->2024): {len(escolas_2024 - escolas_2022):,}")


def figura_universo_por_ano(df: pd.DataFrame) -> None:
...
def analise_perfil_universo(df: pd.DataFrame) -> None:
    ...
    relatorio_dimensoes_painel(df)
    relatorio_estabilidade_longitudinal(df)
    figura_universo_por_ano(df)
...
def relatorio_localizacao(df24: pd.DataFrame) -> pd.Series:
    print("\nLocalização urbana/rural (2024):")
    localizacao = df24["localizacao_txt"].value_counts()
    for tipo, n in localizacao.items():
        pct = n / len(df24) * 100
        print(f"  {tipo}: {n:,} ({pct:.1f}%)")
    return localizacao


def relatorio_mesorregioes(df24: pd.DataFrame) -> pd.DataFrame:
...
def relatorio_top_municipios(df24: pd.DataFrame) -> None:
...
def figura_distribuicao_geografica(localizacao: pd.Series, meso: pd.DataFrame) -> None:
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))

    ax = axes[0]
    sizes = localizacao.values
    labels = [f"{t}\n({n:,})" for t, n in zip(localizacao.index, sizes)]
...
def analise_geografica(df: pd.DataFrame) -> None:
    ...
    localizacao = relatorio_localizacao(df24)
    meso = relatorio_mesorregioes(df24)
    relatorio_top_municipios(df24)
    figura_distribuicao_geografica(localizacao, meso)
...
def relatorio_matriculas_porte(df: pd.DataFrame, df24: pd.DataFrame) -> None:
    print("\nDescritiva de QT_MAT_MED por ano:")
    print(df.groupby("NU_ANO_CENSO")["QT_MAT_MED"].describe().round(1).to_string())

    print("\nDistribuição por porte (2024):")
    for categoria, n in df24["porte_escola"].value_counts().sort_index().items():
        pct = n / len(df24) * 100
        print(f"  {categoria}: {n:,} ({pct:.1f}%)")

    print("\nEstatísticas de razões operacionais (2024):")
    for razao in ["alunos_por_turma", "alunos_por_docente", "pct_integral"]:
        if razao in df24.columns:
            s = df24[razao].dropna().describe().round(2)
            print(f"\n  {razao}:")
            print(f"    média={s['mean']:.1f}  mediana={s['50%']:.1f}  "
                  f"dp={s['std']:.1f}  min={s['min']:.1f}  max={s['max']:.1f}")


def figura_matriculas_porte(df: pd.DataFrame, df24: pd.DataFrame, mw_p: float) -> None:
    ...
    bp = ax.boxplot(grupos, tick_labels=["Urbana", "Rural"], patch_artist=True,
...
def analise_matriculas_porte(df: pd.DataFrame) -> None:
    ...
    relatorio_matriculas_porte(df, df24)

    urbana = df24[df24["localizacao_txt"] == "Urbana"]["QT_MAT_MED"].dropna()
    rural = df24[df24["localizacao_txt"] == "Rural"]["QT_MAT_MED"].dropna()
    mw_stat, mw_p = stats.mannwhitneyu(urbana, rural, alternative="two-sided")

    figura_matriculas_porte(df, df24, mw_p)
...
def prevalencia_infra_por_ano(df: pd.DataFrame, cols_infra: list[str]) -> dict[int, pd.Series]:
    return {
        ano: (df[df["NU_ANO_CENSO"] == ano][cols_infra].mean() * 100).round(1)
        for ano in ANOS_CENSO
    }


def relatorio_prevalencia_infra(prevalencia: dict[int, pd.Series], cols_infra: list[str]) -> None:
    print("\n% de escolas com cada item de infraestrutura:")
    tabela = pd.DataFrame(prevalencia)
...
def relatorio_indice_infra(df24: pd.DataFrame) -> tuple[pd.Series, pd.Series, float]:
    ...
    urbana = df24[df24["localizacao_txt"] == "Urbana"]["indice_infra"].dropna()
    rural = df24[df24["localizacao_txt"] == "Rural"]["indice_infra"].dropna()
    print(f"\n  Infraestrutura média Urbana: {urbana.mean():.3f}")
    print(f"  Infraestrutura média Rural: {rural.mean():.3f}")
    t_stat, t_p = stats.ttest_ind(urbana, rural, equal_var=False)
    ...
    return urbana, rural, t_p


def figura_infraestrutura(
    prevalencia: dict[int, pd.Series],
    cols_infra: list[str],
    urbana: pd.Series,
    rural: pd.Series,
    t_p: float,
    ...
    ax = axes[0]
    data_hm = pd.DataFrame(prevalencia)
    ...
    ax = axes[1]
    bp = ax.boxplot(
        [urbana, rural], tick_labels=["Urbana", "Rural"],
        patch_artist=True,
        medianprops={"color": "white", "linewidth": 2},
    )
    for patch, cor in zip(bp["boxes"], [COR_URBANA, COR_RURAL]):
...
def figura_comparacao_infra(prevalencia: dict[int, pd.Series], cols_infra: list[str]) -> None:
    fig, ax = plt.subplots(figsize=(10, 7))
    y = np.arange(len(cols_infra))
    v2022 = [prevalencia[2022][c] for c in cols_infra]
    v2024 = [prevalencia[2024][c] for c in cols_infra]
...
def analise_infraestrutura(df: pd.DataFrame) -> None:
    ...
    prevalencia = prevalencia_infra_por_ano(df, cols_infra)
    relatorio_prevalencia_infra(prevalencia, cols_infra)

    df24 = df[df["NU_ANO_CENSO"] == 2024]
    urbana, rural, t_p = relatorio_indice_infra(df24)

    figura_infraestrutura(prevalencia, cols_infra, urbana, rural, t_p)
    figura_comparacao_infra(prevalencia, cols_infra)
...
def relatorio_conectividade(df24: pd.DataFrame, cols_tec: list[str]) -> None:
    print("\nConectividade (2024):")
    for c in cols_tec:
        n_sim = df24[c].sum()
...
def figura_evolucao_conectividade(df: pd.DataFrame, cols_tec: list[str]) -> None:
    dados_tec = {}
    for c in cols_tec:
    ...
    for i, (label, vals) in enumerate(dados_tec.items()):
        ys = [vals[a] for a in ANOS_CENSO]
        ax.plot(ANOS_CENSO, ys, marker=marcadores[i % 3], linewidth=2,
                markersize=7, label=label)
        for ano, y in zip(ANOS_CENSO, ys):
...
def analise_tecnologia(df: pd.DataFrame) -> None:
    ...
    df24 = df[df["NU_ANO_CENSO"] == 2024]
    cols_tec = [c for c in ["IN_INTERNET", "IN_INTERNET_ALUNOS", "IN_BANDA_LARGA"]
                if c in df24.columns]

    relatorio_conectividade(df24, cols_tec)
    figura_evolucao_conectividade(df, cols_tec)
...
def relatorio_tendencias(df: pd.DataFrame, cols_operacionais: list[str]) -> None:
    print("\nMédia das variáveis operacionais por ano:")
    tab = df.groupby("NU_ANO_CENSO")[cols_operacionais].mean().round(2)
    print(tab.to_string())
    ...
    grupos_mat = [df[df["NU_ANO_CENSO"] == a]["QT_MAT_MED"].dropna() for a in ANOS_CENSO]
    kw_stat, kw_p = stats.kruskal(*grupos_mat)
    print(f"\n[Kruskal-Wallis QT_MAT_MED entre anos] H={kw_stat:.2f}, p={kw_p:.4f}")


def figura_tendencias_temporais(df: pd.DataFrame) -> None:
    pares = [
    ...
    for ax, (col, titulo) in zip(axes.flatten(), pares):
        ...
        ax.errorbar(ANOS_CENSO, [medias.get(a, np.nan) for a in ANOS_CENSO],
                    yerr=[desvios.get(a, 0) for a in ANOS_CENSO],
...
def analise_temporal(df: pd.DataFrame) -> None:
    ...
    cols_operacionais = [c for c in ["QT_MAT_MED", "QT_TUR_MED", "QT_DOC_MED",
                                     "alunos_por_turma", "alunos_por_docente", "pct_integral"]
                         if c in df.columns]
    relatorio_tendencias(df, cols_operacionais)
    figura_tendencias_temporais(df)
...
def relatorio_correlacao_infra(df24: pd.DataFrame) -> None:
    print("\nCorrelação (Spearman) com índice_infra:")
    for c in [c for c in COLUNAS_INFRA if c in df24.columns]:
        r, p = stats.spearmanr(df24["indice_infra"], df24[c], nan_policy="omit")
        sig = "*" if p < 0.05 else ""
        print(f"  {COLUNAS_INFRA_LABELS.get(c, c):30s}  r={r:+.3f}  p={p:.4f}{sig}")


def figura_heatmap_correlacoes(corr: pd.DataFrame) -> None:
    labels_plot = {
...
def analise_correlacoes(df: pd.DataFrame) -> None:
    ...
    relatorio_correlacao_infra(df24)
    figura_heatmap_correlacoes(corr)
...
def relatorio_tempo_integral(df: pd.DataFrame, df24: pd.DataFrame) -> None:
...
def figura_tempo_integral(df: pd.DataFrame) -> None:
    fig, axes = plt.subplots(1, 2, figsize=(13, 5))
    ...
    bars = ax.bar(ANOS_CENSO, props,
                  color=[CORES_ANOS[a] for a in ANOS_CENSO],
...
def analise_tempo_integral(df: pd.DataFrame) -> None:
    ...
    df24 = df[df["NU_ANO_CENSO"] == 2024]
    relatorio_tempo_integral(df, df24)
    figura_tempo_integral(df)
...
def montar_ranking_municipios(df24: pd.DataFrame) -> pd.DataFrame:
...
def relatorio_ranking_municipios(ranking: pd.DataFrame) -> None:
    print("\nTop 20 municípios por total de matrículas de EM (2024):")
    print(ranking.head(20).to_string())

    com_minimo = ranking[ranking["n_escolas"] >= 3]
    print("\nTop 10 municípios com MENOR infraestrutura média (2024, mín. 3 escolas):")
    baixa_infra = com_minimo.sort_values("media_infra").head(10)
    ...
    alta_infra = com_minimo.sort_values("media_infra", ascending=False).head(10)
    print(alta_infra[["n_escolas", "media_infra", "pct_rural"]].to_string())


def figura_ranking_municipios(ranking: pd.DataFrame) -> None:
    fig, ax = plt.subplots(figsize=(10, 7))
...
def analise_ranking_municipios(df: pd.DataFrame) -> None:
    ...
    df24 = df[df["NU_ANO_CENSO"] == 2024]
    ranking = montar_ranking_municipios(df24)
    relatorio_ranking_municipios(ranking)
    figura_ranking_municipios(ranking)
